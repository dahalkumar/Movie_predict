"""
author @ kumar dahal
this class is writen to create the folder structure of projects
"""

from movie_prediction.entity.config_entity import DataIngestionConfig, DataTransformationConfig,DataValidationConfig,   \
ModelTrainerConfig,ModelEvaluationConfig,ModelPusherConfig,TrainingPipelineConfig
from movie_prediction.utils.common import read_yaml_file
...
class Configuartion:
    ...
    def get_model_evaluation_config(self) ->ModelEvaluationConfig:
        try:
            model_evaluation_config = self.config_info[MODEL_EVALUATION_CONFIG_KEY]  #artifact_dir
            artifact_dir = os.path.join(self.training_pipeline_config.artifact_dir,  #model_evaluation
                                        MODEL_EVALUATION_ARTIFACT_DIR, )

            model_evaluation_file_path = os.path.join(artifact_dir,
                                                    model_evaluation_config[MODEL_EVALUATION_FILE_NAME_KEY])#model_evaluation.yaml
    ...
    def get_model_pusher_config(self) -> ModelPusherConfig:
        try:
            #get the current time stamp from constant
            time_stamp = f"{datetime.now().strftime('%Y%m%d%H%M%S')}"
            
            #access the model_pusher_config to access other info
            model_pusher_config_info = self.config_info[MODEL_PUSHER_CONFIG_KEY]
            #create the dir /current_working_dir/saved_models/time_stamp
            export_dir_path = os.path.join(ROOT_DIR, model_pusher_config_info[MODEL_PUSHER_MODEL_EXPORT_DIR_KEY],
                                           time_stamp)

            #assign it to the input which is our config_entity
    ...
    def get_training_pipeline_config(self) ->TrainingPipelineConfig:
        try:
            ...
            training_pipeline_config[TRAINING_PIPELINE_NAME_KEY],      #Movies
